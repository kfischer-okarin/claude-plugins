...
Run the bundled script to collect all feedback sources into one labeled stream —
the local `.review.json` (produced by the `my-review` skill), inline review
comments, review summaries, and conversation (issue) comments. Pass the PR
number from `$ARGUMENTS` if given; otherwise it defaults to the current branch's
PR.
...
List every comment from both sources with its file, line, author, and body. For
each, give a quick verdict on whether it's worth applying — weighing
